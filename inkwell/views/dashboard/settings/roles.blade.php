<div>
    ...
    <div class="mb-6 flex justify-end">
        <button wire:click="$toggle('showCreate')" class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">
            {{ $editingRole ? 'Cancel Edit' : 'Add Role' }}
        </button>
    </div>

    @if($showCreate)
        <div class="mb-6 bg-white rounded-lg border border-gray-200 p-6">
            <h3 class="font-medium text-gray-900 mb-4">{{ $editingRole ? 'Edit Role' : 'Create Role' }}</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($permissions as $group => $groupPerms)
                            <div class="bg-gray-50 rounded-lg p-4">
                                <div class="font-medium text-gray-900 mb-2 capitalize">{{ $group }}</div>
                                <div class="space-y-2">
                                    @foreach($groupPerms as $permission)
                                        <label class="flex items-center">
                                            <input wire:model="selectedPermissions" type="checkbox" value="{{ $permission->id }}" class="rounded border-gray-300">
                                            <span class="ml-2 text-sm text-gray-700">{{ explode('.', $permission->slug)[1] ?? $permission->slug }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($roles as $role)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{{ $role->slug }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button wire:click="edit({{ $role->id }})" class="text-blue-600 hover:text-blue-900 mr-4">Edit</button>
                            <button wire:click="delete({{ $role->id }})" wire:confirm="Are you sure?" class="text-red-600 hover:text-red-900">Delete</button>
                        </td>
                    </tr>
                @empty
                    <tr>
